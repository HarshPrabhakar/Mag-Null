...
class SwarmPanel(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        if self.score >= 68:
            pulse_alpha = int(180 + 75 * math.sin(self._pulse))
            col.setAlpha(pulse_alpha)
        ...
        for i, (name, key, max_val) in enumerate(factor_defs):
            ...
            # Label
            p.setPen(QColor(C_DIM))
            p.drawText(bar_x + bar_w + 6, by + 9,
                       f"{name}  {val:.0f}/{max_val}")


# ══════════════════════════════════════════════════════════════════
# Pipeline Panel — 7 stage indicators
# ══════════════════════════════════════════════════════════════════
STAGE_NAMES = [
    "SDR SCAN", "FFT", "NOISE EST.",
    "ENERGY DET.", "CLASSIFIER", "SWARM", "RF SILENCE"
]
...
class PipelinePanel(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        for i, (name, flag) in enumerate(zip(STAGE_NAMES, self.flags)):
            ...
            fm = p.fontMetrics()
            p.drawText(x - fm.horizontalAdvance(name)//2,
                       cy + 16, name)
...
class AlertLog(QWidget):
    def __init__(self, parent=None):
        ...
        hdr = QLabel("  SYSTEM ALERTS")
        hdr.setStyleSheet(f"background:{C_BG2};color:{C_DIM};font-size:9px;font-weight:600;"
                          f"letter-spacing:2px;padding:4px 0;border-bottom:1px solid {C_BORDER};")
    ...
    def push_alerts(self, alerts):
        for a in alerts:
            ...
            lbl.setStyleSheet(f"color:{col};font-family:'Courier New';font-size:9px;"
                              f"padding:1px 0;background:{C_BG};")
            self.inner_layout.insertWidget(
                self.inner_layout.count() - 1, lbl)
...
class SilenceBanner(QWidget):
    def __init__(self, parent=None):
        ...
        self.lbl.setStyleSheet(f"font-family:'Courier New';font-weight:700;"
    ...
    def _tick(self):
        ...
        if self.height() > 0:
            ...
            self.setStyleSheet(
                f"background: rgba(255,51,85,{alpha//255:.2f});border-bottom:1px solid {C_RED};")
...
class MainWindow(QMainWindow):
    def __init__(self, pipeline, bridge):
        super().__init__()
    ...
    def _build_ui(self):
        ...
        # Top: waterfall left | contacts right
        ...
        wf_hdr = self._panel_label("SPECTROGRAM  ·  2.4 GHz ISM BAND")
        left_v.addWidget(wf_hdr)
        self.waterfall = WaterfallWidget()
        left_v.addWidget(self.waterfall, 3)
        self.spectrum_w = SpectrumWidget()
        left_v.addWidget(self.spectrum_w)

        ct_hdr = self._panel_label("THREAT CONTACTS")
        left_v.addWidget(ct_hdr)
        ...
        right_v.addWidget(self.alert_log)
        ...
        sb = QStatusBar()
        sb.showMessage(
            "Pipeline running at 13 Hz  |  No SDR hardware connected  |  Simulation mode")
    ...
    def _switch(self, sc):
        ...
        for k, b in self._scenario_btns.items():
            b.setObjectName("active" if k == sc else "")
            b.setStyle(b.style())
        descs = {
        ...
        self._sb.showMessage(f"Scenario: {sc.upper()} loaded  |  13 Hz  |  Simulation mode")
    ...
    def _on_tick(self, state):
        ...
        self.cnt_lbl.setStyleSheet(
            f"color:{C_RED};" if n > 0 else f"color:{C_BLUE};")
